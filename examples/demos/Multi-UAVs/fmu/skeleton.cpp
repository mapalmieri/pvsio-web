...
void doStep(const char* action) { 	
	...
	convertDoubletoString(index_state,fmiBuffer.realBuffer[1],1);
	
	
    index_state=findVariable("z1",state);
	convertDoubletoString(index_state,fmiBuffer.realBuffer[2],2);
	
	
    index_state=findVariable("x2",state);
	convertDoubletoString(index_state,fmiBuffer.realBuffer[3],3);
...
void convertNotation(const char name[], int buffer_index){
	...
	//fmiprintf("%s\n", r2);
	if(buffer_index !=10)r2[strlen(r2)-1]='\0'; // removes comma
//	std::string str(r2);
//	std::cout << str << "\n";
