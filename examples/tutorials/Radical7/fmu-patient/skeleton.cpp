 ...
 */
    ...
    index_state=findVariable("Spo2_output",state);
    if ( index_state != -1){
    ...
    index_state=findVariable("hr_output",state);
    if ( index_state != -1){
		writeOutputVariableDouble(index_state,10);
	}
	
	
    index_state=findVariable("HR_max",state);
    if ( index_state != -1){
		writeOutputVariableDouble(index_state,11);
	}
	
	
    index_state=findVariable("HR_min",state);
    if ( index_state != -1){
		writeOutputVariableDouble(index_state,12);
	}
    ...
    index_state=findVariable("HR_fail",state);
    if ( index_state != -1){
	writeOutputVariableBool(index_state,15);
	}
	
    index_state=findVariable("isOn",state);
    if ( index_state != -1){
	writeOutputVariableBool(index_state,16);
	}
    ...
    if (index_state != -1){
	writeOutputVariableString(index_state,7);
	}
	
    index_state=findVariable("HR_label",state);
    if (index_state != -1){
	writeOutputVariableString(index_state,13);
	}
	
    index_state=findVariable("HR_alarm",state);
    if (index_state != -1){
	writeOutputVariableString(index_state,14);
	}
	...
	strcpy(fmiBuffer.r[17],state);
	fmiBuffer.stringBuffer[17]=fmiBuffer.r[17];
...
void doStep() { 	
    ...
    index_state=findVariable("HR",state);
    if ( index_state != -1){
...
void retrieveExternalVariables(){
	 memset(externalvariables,0,MAX_MEMORY); // we need to clean the variables before reusing them
	    ...
	    }if( findVariable("Spo2_output", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# Spo2_output := %f\n",fmiBuffer.realBuffer[3]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",Spo2_output := %f\n",fmiBuffer.realBuffer[3]);
	    ...
	    }if( findVariable("HR", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# HR := %f\n",fmiBuffer.realBuffer[9]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",HR := %f\n",fmiBuffer.realBuffer[9]);
	    ...
	    }if( findVariable("hr_output", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# hr_output := %f\n",fmiBuffer.realBuffer[10]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",hr_output := %f\n",fmiBuffer.realBuffer[10]);
	    ...
	    }if( findVariable("HR_max", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# HR_max := %f\n",fmiBuffer.realBuffer[11]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",HR_max := %f\n",fmiBuffer.realBuffer[11]);
	    ...
	    }if( findVariable("HR_min", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# HR_min := %f\n",fmiBuffer.realBuffer[12]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",HR_min := %f\n",fmiBuffer.realBuffer[12]);
	    ...
	    }if( findVariable("HR_label", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# HR_label := %f\n",fmiBuffer.realBuffer[13]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",HR_label := %f\n",fmiBuffer.realBuffer[13]);
	    ...
	    }if( findVariable("HR_alarm", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# HR_alarm := %f\n",fmiBuffer.realBuffer[14]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",HR_alarm := %f\n",fmiBuffer.realBuffer[14]);
	    ...
	    }if( findVariable("HR_fail", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# HR_fail := %f\n",fmiBuffer.realBuffer[15]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",HR_fail := %f\n",fmiBuffer.realBuffer[15]);
