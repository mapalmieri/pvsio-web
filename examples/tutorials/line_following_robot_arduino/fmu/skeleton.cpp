 ...
 */
	...
	strcat(a,"pvsio");
	strcat(b,"line_following_robot_arduino.pvs");
	strcpy(e,c);
    ...
    index_state=findVariable("out1",state);
    if ( index_state != -1){
		writeOutputVariableDouble(index_state,1);
	}
	
	
    index_state=findVariable("out2",state);
    if ( index_state != -1){
		writeOutputVariableDouble(index_state,2);
	}
	
	
    index_state=findVariable("out3",state);
    if ( index_state != -1){
		writeOutputVariableDouble(index_state,3);
	}
	
	
    index_state=findVariable("out4",state);
    if ( index_state != -1){
    ...
    if ( index_state != -1){
		writeOutputVariableDouble(index_state,5);
	}
	
	
    index_state=findVariable("servoRightVal",state);
    if ( index_state != -1){
		writeOutputVariableDouble(index_state,6);
	}
    ...
    index_state=findVariable("previous_mode",state);
    if (index_state != -1){
	writeOutputVariableString(index_state,13);
	}
	...
	strcpy(fmiBuffer.r[20],state);
	fmiBuffer.stringBuffer[20]=fmiBuffer.r[20];
...
void doStep() { 	
    ...
    index_state=findVariable("backwardRotate",state);
    if ( index_state != -1){
	readInputVariableDouble(index_state,fmiBuffer.realBuffer[9],9);
	}
	
	
    index_state=findVariable("forwardRotate",state);
    if ( index_state != -1){
	readInputVariableDouble(index_state,fmiBuffer.realBuffer[10],10);
	}
	
	
    index_state=findVariable("forwardSpeed",state);
    if ( index_state != -1){
	readInputVariableDouble(index_state,fmiBuffer.realBuffer[11],11);
	}
    ...
    if ( index_state != -1){
	readInputVariableDouble(index_state,fmiBuffer.realBuffer[14],14);
	}
	
	
    index_state=findVariable("angular",state);
    if ( index_state != -1){
	readInputVariableDouble(index_state,fmiBuffer.realBuffer[15],15);
	}
	
	
    index_state=findVariable("posx",state);
    if ( index_state != -1){
	readInputVariableDouble(index_state,fmiBuffer.realBuffer[16],16);
	}
	
	
    index_state=findVariable("posy",state);
    if ( index_state != -1){
	readInputVariableDouble(index_state,fmiBuffer.realBuffer[17],17);
	}
	
	
    index_state=findVariable("left_rotation",state);
    if ( index_state != -1){
	readInputVariableDouble(index_state,fmiBuffer.realBuffer[18],18);
	}
	
	
    index_state=findVariable("right_rotation",state);
    if ( index_state != -1){
	readInputVariableDouble(index_state,fmiBuffer.realBuffer[19],19);
	}
...
void retrieveExternalVariables(){
	 memset(externalvariables,0,MAX_MEMORY); // we need to clean the variables before reusing them
	    ...
	    }if( findVariable("out2", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# out2 := %f\n",fmiBuffer.realBuffer[2]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",out2 := %f\n",fmiBuffer.realBuffer[2]);
	    ...
	    }if( findVariable("out3", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# out3 := %f\n",fmiBuffer.realBuffer[3]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",out3 := %f\n",fmiBuffer.realBuffer[3]);
	    ...
	    }if( findVariable("out4", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# out4 := %f\n",fmiBuffer.realBuffer[4]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",out4 := %f\n",fmiBuffer.realBuffer[4]);
	    ...
	    }if( findVariable("servoLeftVal", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# servoLeftVal := %f\n",fmiBuffer.realBuffer[5]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",servoLeftVal := %f\n",fmiBuffer.realBuffer[5]);
	    ...
	    }if( findVariable("servoRightVal", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# servoRightVal := %f\n",fmiBuffer.realBuffer[6]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",servoRightVal := %f\n",fmiBuffer.realBuffer[6]);
	    ...
	    }if( findVariable("lfLeftVal", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# lfLeftVal := %f\n",fmiBuffer.realBuffer[7]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",lfLeftVal := %f\n",fmiBuffer.realBuffer[7]);
	    ...
	    }if( findVariable("lfRightVal", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# lfRightVal := %f\n",fmiBuffer.realBuffer[8]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",lfRightVal := %f\n",fmiBuffer.realBuffer[8]);
	    ...
	    }if( findVariable("backwardRotate", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# backwardRotate := %f\n",fmiBuffer.realBuffer[9]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",backwardRotate := %f\n",fmiBuffer.realBuffer[9]);
	    ...
	    }if( findVariable("forwardRotate", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# forwardRotate := %f\n",fmiBuffer.realBuffer[10]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",forwardRotate := %f\n",fmiBuffer.realBuffer[10]);
	    ...
	    }if( findVariable("forwardSpeed", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# forwardSpeed := %f\n",fmiBuffer.realBuffer[11]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",forwardSpeed := %f\n",fmiBuffer.realBuffer[11]);
	    ...
	    }if( findVariable("previous_mode", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# previous_mode := %f\n",fmiBuffer.realBuffer[13]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",previous_mode := %f\n",fmiBuffer.realBuffer[13]);
