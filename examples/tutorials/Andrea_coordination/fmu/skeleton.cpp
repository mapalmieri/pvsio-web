 ...
 */
    ...
    index_state=findVariable("timesteps",state);
    if ( index_state != -1){
    ...
    index_state=findVariable("st",state);
    if ( index_state != -1){
		index_state1=findVariable("self", &state[index_state]);
		writeOutputVariableDouble(index_state1+index_state,3);
	}
	...
	strcpy(fmiBuffer.r[6],state);
	fmiBuffer.stringBuffer[6]=fmiBuffer.r[6];
...
void doStep() { 	
    ...
    index_state=findVariable("st",state);
    if ( index_state != -1){
		index_state1=findVariable("prec", &state[index_state]);
		readInputVariableDouble(index_state1+index_state,fmiBuffer.realBuffer[2],2);
	}
    ...
    index_state=findVariable("st",state);
    if ( index_state != -1){
		index_state1=findVariable("foll", &state[index_state]);
		readInputVariableDouble(index_state1+index_state,fmiBuffer.realBuffer[4],4);
	}
...
void retrieveExternalVariables(){
	 memset(externalvariables,0,MAX_MEMORY); // we need to clean the variables before reusing them
	    if( findVariable("timesteps", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# timesteps := %f\n",fmiBuffer.realBuffer[1]); 
