...
void* watchdog_function(void *arg){
	...
	start = clock();
	while((clock()-start)/CLOCKS_PER_SEC < MAX_TIME){/*do nothing*/}
 ...
 */
	...
	strcat(c,"/");
	strcpy(e,c);
	strcat(c, "pvs-6.0-ix86_64-Linux-sbclisp/pvsio");
	strcat(a,"pvsio");
	...
	chdir("pvs-6.0-ix86_64-Linux-sbclisp/");
	usleep(10);
	...
	PVSioPrint();
	PVSioEval("click_FMI_btn_key1");
	PVSioPrint();
	PVSioEval("click_FMI_btn_key2");
	PVSioPrint();
	PVSioEval("click_FMI_btn_key3");
	PVSioPrint();
	PVSioEval("click_FMI_btn_fup");
	PVSioPrint();
	PVSioEval("click_FMI_btn_sup");
	PVSioPrint();
	PVSioEval("click_FMI_btn_run");
	PVSioPrint();
	PVSioEval("click_FMI_btn_on");
	PVSioPrint();
	PVSioEval("FMItick");
	PVSioPrint();
    ...
    index_state=findVariable("outputrate",state);
    if ( index_state != -1){
		writeOutputVariableDouble(index_state,45);
	}
    ...
    if ( index_state != -1){
		...
		writeOutputVariableDouble(index_state1+index_state,49);
	}
    ...
    if ( index_state != -1){
		...
		writeOutputVariableDouble(index_state1+index_state,51);
	}
    ...
    if ( index_state != -1){
		...
		writeOutputVariableDouble(index_state1+index_state,53);
	}
    ...
    if ( index_state != -1){
		...
		writeOutputVariableDouble(index_state1+index_state,58);
	}
    ...
    if ( index_state != -1){
		...
		writeOutputVariableBool(index_state1+index_state,46);
	}
    ...
    if ( index_state != -1){
		...
		writeOutputVariableBool(index_state1+index_state,50);
	}
    ...
    if ( index_state != -1){
		...
		writeOutputVariableBool(index_state1+index_state,52);
	}
    ...
    if ( index_state != -1){
		...
		writeOutputVariableBool(index_state1+index_state,55);
	}
	...
	strcpy(fmiBuffer.r[59],state);
	fmiBuffer.stringBuffer[59]=fmiBuffer.r[59];
...
void retrieveExternalVariables(){
	 memset(externalvariables,0,MAX_MEMORY); // we need to clean the variables before reusing them
	    ...
	    }if( findVariable("outputrate", state)== -1){
			if ( strlen(externalvariables) == 0) {
				sprintf(tempvar,";(# outputrate := %f\n",fmiBuffer.realBuffer[45]); 
				strcat(externalvariables,tempvar);
				}
			else {
				sprintf(tempvar,",outputrate := %f\n",fmiBuffer.realBuffer[45]);
