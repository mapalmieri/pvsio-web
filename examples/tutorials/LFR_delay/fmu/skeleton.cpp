 ...
 */
    ...
    index_state=findVariable("motorSpeedOut",state);
    if ( index_state != -1){
    ...
    if ( index_state != -1){
		...
		writeOutputVariableDouble(index_state1+index_state,4);
	}
	...
	strcpy(fmiBuffer.r[5],state);
	fmiBuffer.stringBuffer[5]=fmiBuffer.r[5];
...
void doStep() { 	
    ...
    index_state=findVariable("motorSpeedIn",state);
    if ( index_state != -1){
		index_state1=findVariable("left", &state[index_state]);
		readInputVariableDouble(index_state1+index_state,fmiBuffer.realBuffer[1],1);
	}
