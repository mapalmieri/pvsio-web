...
void initialize(const char* location) {
	...
	convertStateToDouble(index_state,4);
...
void doStep(const char* action) { 	
	...
	convertDoubleToState(index_state,fmiBuffer.realBuffer[1],1);
	
	
    index_state=findVariable("lfRightVal",state);
	convertDoubleToState(index_state,fmiBuffer.realBuffer[2],2);
	...
	convertNotation("servoLeftVal",3);
    index_state=findVariable("servoLeftVal",state);
	convertStateToDouble(index_state,3);
	
	 
	convertNotation("servoRightVal",4);
    index_state=findVariable("servoRightVal",state);
	convertStateToDouble(index_state,4);
...
void convertStateToString(int state_index, int buffer_index){
	sscanf(&state[state_index],"%s",r[buffer_index]);
	r[buffer_index][strlen(r[buffer_index])-1] = '\0';
	//printf("%s\n",r[buffer_index]);
	fmiBuffer.stringBuffer[buffer_index]=r[buffer_index];
