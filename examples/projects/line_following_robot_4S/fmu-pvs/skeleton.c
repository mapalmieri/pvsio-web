...
void initialize(ModelInstance* comp, const char* location) {
    ...
    comp->fmiBuffer.realBuffer[12] = comp->st.servoLeftVal;
    comp->fmiBuffer.realBuffer[13] = comp->st.servoRightVal;
    ...
    comp->fmiBuffer.intBuffer[9] = comp->st.port;
...
void doStep(ModelInstance* comp, const char* action) {
    ...
    comp->st.posx = comp->fmiBuffer.realBuffer[10];
    comp->st.posy = comp->fmiBuffer.realBuffer[11];
...
void stateToString(State st, char* str) {
	...
	strcat(str, temp);
	sprintf(temp, " highRotate := %f,", st.highRotate);
	strcat(str, temp);
	sprintf(temp, " lfFarLeftVal := %f,", st.lfFarLeftVal);
	strcat(str, temp);
	sprintf(temp, " lfFarRightVal := %f,", st.lfFarRightVal);
	strcat(str, temp);
	sprintf(temp, " lfMidLeftVal := %f,", st.lfMidLeftVal);
	strcat(str, temp);
	sprintf(temp, " lfMidRightVal := %f,", st.lfMidRightVal);
	strcat(str, temp);
	sprintf(temp, " lowRotate := %f,", st.lowRotate);
	strcat(str, temp);
	sprintf(temp, " mediumRotate := %f,", st.mediumRotate);
	strcat(str, temp);
